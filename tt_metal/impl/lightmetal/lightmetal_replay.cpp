...
void LightMetalReplay::SetupDevices() {
    ...
    auto devices_map = tt::tt_metal::detail::CreateDevices(
        {mmio_device_id}, 1, DEFAULT_L1_SMALL_SIZE, buffer_size, dispatch_core_type);
...
void LightMetalReplay::Execute(const tt::target::CreateProgramCommand* cmd) {
    log_info(tt::LogMetalTrace, "LightMetalReplay(CreateProgram) global_id: {} ", cmd->global_id());
...
bool LightMetalReplay::ExecuteLightMetalBinary() {
    ...
    try {
        ...
        SetupDevices();
        log_info(tt::LogMetalTrace, "LightMetal Binary has {} cmds, {} traces", commands->size(), trace_descs->size());

        // Just loop over all commands, and execute. This is purposely kept simple for prototyping v0.
        // TODO (kmabee) - should expand to cover, multiple devices, cqs, etc. Downgrade to log_debug
        // once more robustly implemented/tested.
        uint32_t idx = 1;
