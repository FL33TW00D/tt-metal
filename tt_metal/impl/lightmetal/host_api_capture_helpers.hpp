...
// TODO (kmabee) - Consider passing Buffer* to capture funcs intead so it's clear we don't extend lifetime of buffer.
...
inline void CaptureFinish(CommandQueue& cq) {
    auto& ctx = LightMetalCaptureContext::Get();
    uint32_t cq_global_id = cq.id();  // TODO (kmabee) - consider storing/getting CQ from global map instead.
    log_debug(tt::LogMetalTrace, "{}: cq_global_id: {}", __FUNCTION__, cq_global_id);
    auto cmd = tt::target::CreateFinishCommand(ctx.GetBuilder(), cq_global_id);
