...
static constexpr uint32_t NUM_PTR_REGS_PER_INPUT_QUEUE = 1;
static constexpr uint32_t NUM_PTR_REGS_PER_OUTPUT_QUEUE = 2;
...
public:
    template <uint32_t QueueId>
    uint32_t forward_data_from_input(bool, bool) {
        return 0;
    }
};
...
// DisableRemotePtr will disable the remote ptr updates if the implementation uses a custom method.
template <typename T, typename R, bool DisableRemotePtr>
...
// T is the implementation. R is the remote controller for pointer updates.
// DisableRemotePtr will disable the remote ptr updates if the implementation uses a custom method.
template <typename T, typename R, typename InputNetworkTypeSequence, typename InputCBSequence, bool DisableRemotePtr>
...
struct init_params_t {
    ...
    // Output Queue
    PacketInputQueueVariant* input_queues{nullptr};
...
public:
    void init(const init_params_t* params) {
        ...
        if constexpr (!DisableRemotePtr) {
            // If remote ptr will be accessed we need to make sure not to overwrite L1[0]
            WAYPOINT("PQA1");
        ...
        this->remote.init(params->ptrs_addr, params->remote_ptrs_addr);
    ...
    inline uint8_t get_remote_x() { return this->remote_x; }

    inline uint8_t get_remote_y() { return this->remote_y; }
    ...
    inline uint32_t get_remote_ready_status() const {
        return *reinterpret_cast<volatile uint32_t*>(STREAM_REG_ADDR(this->queue_id, STREAM_REMOTE_SRC_REG_INDEX));
    }

    inline bool is_remote_ready() const { return this->get_remote_ready_status() == PACKET_QUEUE_REMOTE_READY_FLAG; }
    ...
    inline void reset_ready_flag() {
        *reinterpret_cast<volatile uint32_t*>(STREAM_REG_ADDR(this->queue_id, STREAM_REMOTE_SRC_REG_INDEX)) = 0;
    }

    inline void set_queue_finished() {
        *reinterpret_cast<volatile uint32_t*>(STREAM_REG_ADDR(this->queue_id, STREAM_REMOTE_SRC_REG_INDEX)) =
            PACKET_QUEUE_REMOTE_FINISHED_FLAG;
    }

    inline void set_remote_queue(uint8_t remote_queue_id, uint32_t remote_x, uint32_t remote_y) {
    ...
    inline void send_remote_finished_notification() {
        this->remote.reg_update(
            STREAM_REG_ADDR(this->remote_queue_id, STREAM_REMOTE_SRC_REG_INDEX),
            PACKET_QUEUE_REMOTE_FINISHED_FLAG,
            this->remote_x,
            this->remote_y);
    }
    ...
    inline void advance_queue_local_rptr_cleared(uint32_t num_words) {
        this->impl()->_advance_queue_local_rptr_cleared(num_words);
    ...
    inline void advance_queue_remote_wptr(uint32_t num_words) {
        if constexpr (!DisableRemotePtr) {
            ...
            this->remote.reg_update(reg_addr, val, this->remote_x, this->remote_y);
    ...
    inline uint32_t cb_mode_get_local_sem_val(uint32_t sem_id) {
        invalidate_l1_cache();
    ...
    inline bool cb_mode_local_sem_downstream_complete(uint32_t sem_id) {
        invalidate_l1_cache();
        volatile tt_l1_ptr uint32_t* local_sem_addr =
            reinterpret_cast<volatile tt_l1_ptr uint32_t*>(get_semaphore<fd_core_type>(sem_id));
        // semaphore underflow is currently used to signal path teardown with minimal prefetcher changes
    ...
    inline void cb_mode_inc_local_sem_val(uint32_t sem_id, uint32_t val) {
        const auto sem_l1_addr = get_semaphore<fd_core_type>(sem_id);
        const auto sem_noc_addr = get_noc_addr(sem_l1_addr);
        noc_semaphore_inc(sem_noc_addr, val);
        noc_async_atomic_barrier();
    }

    inline void cb_mode_inc_remote_sem_val(uint32_t sem_id, uint32_t val) {
        const auto sem_l1_addr = get_semaphore<fd_core_type>(sem_id);
        const auto sem_noc_addr = get_noc_addr(this->get_remote_x(), this->get_remote_y(), sem_l1_addr);
        if (val > 0) {
            noc_semaphore_inc(sem_noc_addr, val);
        }
    }

    inline uint32_t cb_mode_rptr_sent_advance_page_align(uint32_t page_size_words) {
        const auto rptr_val = this->get_queue_local_rptr_sent();
        const auto page_size_words_mask = page_size_words - 1;
        const auto num_words_past_page_boundary = rptr_val & page_size_words_mask;
        uint32_t input_pad_words_skipped = 0;
        if (num_words_past_page_boundary > 0) {
            input_pad_words_skipped = page_size_words - num_words_past_page_boundary;
            this->advance_queue_local_rptr_sent(input_pad_words_skipped);
        }
        return input_pad_words_skipped;
    }

    inline void cb_mode_local_sem_wptr_update(uint32_t sem_id, uint32_t page_size_words) {
        const auto local_sem_val = this->cb_mode_get_local_sem_val(sem_id);
        for (uint32_t i = 0; i < local_sem_val; ++i) {
            this->advance_queue_local_wptr(page_size_words);
        }
        this->cb_mode_inc_local_sem_val(sem_id, -local_sem_val);
    }

    inline void cb_mode_local_sem_rptr_cleared_update(uint32_t sem_id, uint32_t page_size_words) {
        const auto local_sem_val = this->cb_mode_get_local_sem_val(sem_id);
        for (uint32_t i = 0; i < local_sem_val; ++i) {
            this->advance_queue_local_rptr_cleared(page_size_words);
        }
        this->cb_mode_inc_local_sem_val(sem_id, -local_sem_val);
    }
};  // packet_queue_base_t
...
template <typename T, typename R, bool DisableRemotePtr>
class packet_queue_input_t
    : public packet_queue_base_t<packet_queue_input_t<T, R, DisableRemotePtr>, R, DisableRemotePtr> {
private:
    friend packet_queue_base_t<packet_queue_input_t<T, R, DisableRemotePtr>, R, DisableRemotePtr>;
    ...
    inline void start_next_packet() {
        ...
        const uint32_t next_header_addr =
            (this->get_queue_start_addr_words() + this->get_queue_rptr_sent_offset_words()) * PACKET_WORD_SIZE_BYTES;
        ...
        // Set this to true right now so if the downstream _set_next_packet will not call
        // start_next_packet again
...
protected:
    ...
    void _advance_queue_local_wptr(uint32_t num_words) {
        *this->local_wptr_update = num_words << REMOTE_DEST_BUF_WORDS_FREE_INC;
    }

    void _advance_queue_local_rptr_sent(uint32_t num_words) { this->local_rptr_sent += num_words; }

    void _advance_queue_local_rptr_cleared(uint32_t num_words) { this->local_rptr_cleared += num_words; }
    ...
    void _init(const init_params_t* params) {
        ...
        this->local_wptr_val = reinterpret_cast<volatile uint32_t*>(STREAM_REG_ADDR(
            NUM_PTR_REGS_PER_INPUT_QUEUE * params->queue_id, STREAM_REMOTE_DEST_BUF_SPACE_AVAILABLE_REG_INDEX));
        ...
        this->local_wptr_update = reinterpret_cast<volatile uint32_t*>(STREAM_REG_ADDR(
            NUM_PTR_REGS_PER_INPUT_QUEUE * params->queue_id, STREAM_REMOTE_DEST_BUF_SPACE_AVAILABLE_UPDATE_REG_INDEX));

        // Reset local wptr
        *reinterpret_cast<volatile uint32_t*>(STREAM_REG_ADDR(
            NUM_PTR_REGS_PER_INPUT_QUEUE * params->queue_id, STREAM_REMOTE_DEST_BUF_SIZE_REG_INDEX)) = 0;
...
public:
    ...
    inline uint16_t get_curr_packet_src() {
        if (!this->curr_packet_valid) {
            this->start_next_packet();
        }
    ...
    inline uint16_t get_curr_packet_dest() {
        if (!this->curr_packet_valid) {
            this->start_next_packet();
        }
    ...
    inline uint16_t get_curr_packet_flags() {
        if (!this->curr_packet_valid) {
            this->start_next_packet();
        }
...
// R is the remote type
template <typename R>
class input_queue_impl_t final : public packet_queue_input_t<input_queue_impl_t<R>, R, false> {
    friend packet_queue_input_t<input_queue_impl_t<R>, R, false>;
...
// R is the remote type
template <typename R>
class input_queue_cb_mode_impl_t final : public packet_queue_input_t<input_queue_cb_mode_impl_t<R>, R, true> {
private:
    friend packet_queue_input_t<input_queue_cb_mode_impl_t<R>, R, true>;
...
protected:
    inline void _init(const init_params_t* params) {
        ...
        this->set_curr_packet_tag(0xabcd);
    ...
    inline void _update_local_wptr_val() {
        this->cb_mode_local_sem_wptr_update(this->config.local_sem_id, this->config.page_size_words);
    }

    inline uint32_t _align_rptr_sent(uint32_t num_words) {
        return this->cb_mode_rptr_sent_advance_page_align(this->config.page_size_words);
    }
    ...
    inline void _align_rptr_cleared(uint32_t num_words) {
        ...
        this->cb_mode_inc_remote_sem_val(this->config.remote_sem_id, remote_sem_inc);
    }
...
template <typename T, typename R, typename InputNetworkTypeSequence, typename InputCBSequence, bool DisableRemotePtr>
class packet_queue_output_t
    : public packet_queue_base_t<
          packet_queue_output_t<T, R, InputNetworkTypeSequence, InputCBSequence, DisableRemotePtr>,
          R,
          DisableRemotePtr> {
private:
    static_assert(InputNetworkTypeSequence::size == InputCBSequence::size);

    friend packet_queue_base_t<
        packet_queue_output_t<T, R, InputNetworkTypeSequence, InputCBSequence, DisableRemotePtr>,
        R,
        DisableRemotePtr>;
    ...
    // template args
    PacketInputQueueVariant* input_queues;
    uint32_t num_input_queues;

    uint32_t words_in_flight[2 * MAX_SWITCH_FAN_IN];  // 2X for curr and prev
...
protected:
    ...
    void _init(const init_params_t* params) {
        WAYPOINT("PQO0");
        ASSERT(params->num_input_queues <= MAX_SWITCH_FAN_IN);
        WAYPOINT("PQO1");

        this->local_wptr_val = &this->local_wptr;
        uint32_t adjusted_queue_id = params->queue_id > 15 ? params->queue_id - 11 : params->queue_id;
        ...
        this->local_rptr_cleared_val = reinterpret_cast<volatile uint32_t*>(STREAM_REG_ADDR(
            NUM_PTR_REGS_PER_OUTPUT_QUEUE * adjusted_queue_id + 1, STREAM_REMOTE_DEST_BUF_SPACE_AVAILABLE_REG_INDEX));
        ...
        // Reset local rptr
        *reinterpret_cast<volatile uint32_t*>(STREAM_REG_ADDR(
            NUM_PTR_REGS_PER_OUTPUT_QUEUE * adjusted_queue_id, STREAM_REMOTE_DEST_BUF_SIZE_REG_INDEX)) = 0;
        *reinterpret_cast<volatile uint32_t*>(STREAM_REG_ADDR(
            NUM_PTR_REGS_PER_OUTPUT_QUEUE * adjusted_queue_id + 1, STREAM_REMOTE_DEST_BUF_SIZE_REG_INDEX)) = 0;
        ...
        this->prev_input_queue_words_in_flight = &(this->words_in_flight[MAX_SWITCH_FAN_IN]);
    ...
    void _advance_queue_local_wptr(uint32_t num_words) { this->local_wptr += num_words; }

    void _advance_queue_local_rptr_sent(uint32_t num_words) {
        *this->local_rptr_sent_update = num_words << REMOTE_DEST_BUF_WORDS_FREE_INC;
    }

    void _advance_queue_local_rptr_cleared(uint32_t num_words) {
    ...
    // Decrement unpacketizer page words sent
    inline void dec_unpacketizer_page_words_sent(uint32_t num_words) {
        this->unpacketizer_page_words_sent -= num_words;
    }
...
public:
    ...
    inline uint32_t get_num_words_to_send() const {
        static_assert(InputQueueIndex < InputNetworkTypeSequence::size);
        static constexpr auto k_max_send_words = remote_max_send_words_v<R>;
        ...
        num_words_to_forward = std::min(num_words_to_forward, k_max_send_words);
    ...
    inline uint32_t forward_data_from_input(bool& full_packet_sent, uint16_t end_of_cmd) {
        ...
        //     (active_input_queue->get_queue_start_addr_words() +
        //     active_input_queue->get_queue_rptr_sent_offset_words()) * PACKET_WORD_SIZE_BYTES;  // Local
        // const uint32_t dest_addr =
        //     (this->get_queue_start_addr_words() + this->get_queue_wptr_offset_words()) * PACKET_WORD_SIZE_BYTES;  //
        //     Remote

        const uint32_t src_addr = active_input_queue->get_queue_rptr_sent_addr_bytes();
        const uint32_t dest_addr = this->get_queue_wptr_addr_bytes();
        this->remote.send_data(src_addr, dest_addr, num_words, this->remote_x, this->remote_y);
...
protected:
    ...
    inline uint32_t _prev_words_in_flight_check_flush() {
        ...
        this->cb_mode_local_sem_rptr_cleared_update(this->config.local_sem_id, this->config.page_size_words);
        return words_flushed;
    }
    ...
    inline void _forward_data_complete(uint32_t words_forwarded, bool full_packet_sent, uint16_t end_of_cmd) {
        ...
        if (full_packet_sent && end_of_cmd) {
            const auto page_size_mask = this->config.page_size_words - 1;
            uint32_t unpacketizer_page_words_sent_past_page_bound =
                this->get_unpacketizer_page_words_sent() & page_size_mask;
        ...
        while (this->get_unpacketizer_page_words_sent() >= this->config.page_size_words) {
            this->dec_unpacketizer_page_words_sent(this->config.page_size_words);
            remote_sem_inc++;
        }
        this->cb_mode_inc_remote_sem_val(this->config.remote_sem_id, remote_sem_inc);
    }
    ...
    inline void _barrier_process() {
        this->cb_mode_local_sem_rptr_cleared_update(this->config.local_sem_id, this->config.page_size_words);
        if (this->cb_mode_local_sem_downstream_complete(this->config.local_sem_id)) {
...
bool wait_all_input_output_ready(
    PacketInputQueueVariant* input_queues, PacketOutputQueueVariant* output_queues, uint32_t timeout_cycles = 0) {
    static_assert(InputNetworkTypeSequence::size == InputCBSequence::size);
