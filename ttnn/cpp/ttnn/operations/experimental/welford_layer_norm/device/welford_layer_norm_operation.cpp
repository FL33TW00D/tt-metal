...
#include "welford_layer_norm_program_factory.hpp"

using namespace tt::tt_metal;

namespace ttnn::operations::experimental::welford_layer_norm {

void WelfordLayerNormDeviceOperation::validate(
    const std::vector<Tensor>& input_tensors,
    const std::vector<std::optional<const Tensor>>& optional_input_tensors) const {
    const auto validateInputTensorCount = [](WelfordLayerNormOpType op_type, uint32_t num_input_tensors) {
        switch (op_type) {
            case WelfordLayerNormOpType::UPDATE:
                TT_FATAL(num_input_tensors == 2, "Expect 2 input tensors for update_cache");
                break;
            case WelfordLayerNormOpType::FUSED_UPDATE:
                TT_FATAL(num_input_tensors == 4, "Expect 4 input tensors for fused_update_cache");
                break;
            case WelfordLayerNormOpType::FILL:
    ...
    if (this->op_type == WelfordLayerNormOpType::UPDATE || this->op_type == WelfordLayerNormOpType::FUSED_UPDATE) {
        ...
        if (this->op_type == WelfordLayerNormOpType::FUSED_UPDATE) {
            validateFusedUpdateTensors(input_tensors.at(1), input_tensors.at(3));
        }
    } else if (this->op_type == WelfordLayerNormOpType::FILL) {
        validateFillOperation(input_tensors.at(0), input_tensors.at(1), input_tensors.at(2), this->batch_idx);
    }
}

std::vector<ttnn::TensorSpec> WelfordLayerNormDeviceOperation::compute_output_specs(
    const std::vector<Tensor>& input_tensors) const {
    // Do nothing because it's an in-place operation
    return {};
}

operation::ProgramWithCallbacks WelfordLayerNormDeviceOperation::create_program(
    const std::vector<Tensor>& input_tensors,
    const std::vector<std::optional<const Tensor>>& optional_input_tensors,
    std::vector<Tensor>& output_tensors) const {
    switch (this->get_parallelization_strategy(input_tensors)) {
        case WelfordLayerNormOpParallelizationStrategy::MULTI_CORE:
        default:
            if (this->op_type == WelfordLayerNormOpType::UPDATE) {
                ...
                return detail::welford_layer_norm_multi_core(
                    cache_tensor,
                    input_tensor,
                    update_idxs_tensor,
                    page_table,
                ...
                std::cout << "Unsupported operation type: " << static_cast<int>(this->op_type) << std::endl;
                const auto& cache_tensor = input_tensors.at(0);
                const auto& input_tensor = input_tensors.at(1);
                const auto update_idxs_tensor =
                    optional_input_tensors.at(0);  // TODO: Is this tensor passed around by value?
...
WelfordLayerNormOpParallelizationStrategy WelfordLayerNormDeviceOperation::get_parallelization_strategy(
    const std::vector<Tensor>& input_tensors) const {
    return WelfordLayerNormOpParallelizationStrategy::MULTI_CORE;
}

operation::Hash WelfordLayerNormDeviceOperation::compute_program_hash(
    const std::vector<Tensor>& input_tensors,
    const std::vector<std::optional<const Tensor>>& optional_input_tensors) const {
    return operation::hash_operation<WelfordLayerNormDeviceOperation>(
